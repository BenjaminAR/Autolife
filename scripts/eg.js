...
const tspan_delta = 16; //name is used as the title for the bubble
...
const use_copy = document.querySelector("use.nav-copy"); //Keeps code DRY avoiding namespace for element creation
...
function addService(serv, index) {
  ...
  let service_bubble = jQuery(".serv-" + index); //Uses tween to look for right position
...
function serviceClick(ev) {
  ...
  let class_index = current_class.split("-")[1]; //Hides current text of the main bubble
